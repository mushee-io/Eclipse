  ...
  return <main className="site-shell">
    <nav className="nav">
      ...
      <div className="wallet-zone">
        ...
        {wrong ? <button className="button small" onClick={() => switchToSepolia().then(sync).catch((e) => setError(friendlyError(e)))}>SWITCH NETWORK</button> : <button className="button small primary" onClick={connect}>{address ? `${address.slice(0, 6)}…${address.slice(-4)}` : "LAUNCH APP"}</button>}
      </div>
    </nav>
