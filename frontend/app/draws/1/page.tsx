...
export default function DrawOne(){
  ...
  const[drawId,setDrawId]=useState<bigint>(1n); const[drawState,setDrawState]=useState<number>(3); const[status,setStatus]=useState(""); const[claimed,setClaimed]=useState(false); const[running,setRunning]=useState(false); const[claiming,setClaiming]=useState(false);
  ...
  const unlock=async()=>{try{setStatus("REQUESTING ACCESS");const u=await account();setStatus("SIGN PRIVACY REQUEST");const r=await unlockDrawResult(u,drawId);setStatus("RESULT READY — CLAIM TO COMPLETE THIS DRAW");setResult(r);setClaimed(false);}catch(e){setStatus(friendlyError(e));}};
  const claim=async()=>{try{setClaiming(true);setStatus("WAITING FOR WALLET — CONFIRM CLAIM IN METAMASK");await claimDrawPrize(result?.id??drawId);setClaimed(true);setStatus("CLAIM TRANSACTION CONFIRMED — YOUR DRAW CYCLE IS COMPLETE");}catch(e){setStatus(friendlyError(e));}finally{setClaiming(false);}};
  ...
  <div className="timeline">{timeline.map(x=><div key={x}><span>✓</span>{x}</div>)}</div><article className="result-card"><p className="eyebrow">YOUR RESULT · DRAW #{drawId.toString()}</p>{result?<><h2>{result.won?"YOU WON.":"NOT THIS TIME."}</h2><p>{result.won?`Private prize: ${result.prize} cUSDT`:"Your principal remains yours."}</p>{claimed?<><button className="button primary" disabled>CLAIMED ✓</button><Link className="button" href="/pool">WITHDRAW PRINCIPAL →</Link><small>Claim confirmed on Sepolia. You can now return to the pool and withdraw your principal.</small></>:<><button className="button primary" disabled={claiming} onClick={claim}>{claiming?"CLAIMING…":"CLAIM RESULT"}</button><small>Every participant uses the same encrypted claim path; a losing allocation settles as encrypted zero.</small></>}</>:<><h2>████████</h2><p>Only your wallet can authorize decryption of its encrypted result.</p><button className="button primary" disabled={drawState!==3} onClick={unlock}>UNLOCK RESULT</button></>} {status&&<small>{status}</small>}</article></section>;
}
