...
import { account, depositCusdt, ethereum, friendlyError, mintTestUsdt, unlockPrincipal, withdrawPrincipal, wrapTestUsdt } from "../../lib/eclipse";
...
export default function Pool() {
  ...
  const connect=()=>run(async()=>{ await ethereum().request({method:"eth_requestAccounts"}); const u=await account(); setUser(u); });
  ...
  {!user && <div className="notice"><span>Connect your wallet to access your confidential position.</span> <button className="button small primary" onClick={connect}>CONNECT WALLET</button></div>}
  ...
  <article className="panel"><p className="eyebrow">WITHDRAW SAVINGS</p><h2>PRINCIPAL IS YOURS</h2><label>AMOUNT · cUSDT<input value={amount} onChange={e=>setAmount(e.target.value)} inputMode="decimal"/></label><p className="muted">The withdrawal request is encrypted before it reaches Eclipse.</p><button className="button full" disabled={!user} onClick={()=>run(async()=>{await withdrawPrincipal(amount,setStep);setBalance(undefined);})}>ENCRYPT & WITHDRAW</button></article></div>
